...
def save_secs_currents(I_timestamp, timestamp, base_path, grid_params):
    """
    Save SECS current amplitudes and grid parameters as CSV files.
    
    Args:
        I_timestamp (numpy.ndarray): SECS current amplitudes
        timestamp (pandas.Timestamp): Timestamp for the data
        base_path (str): Base directory for saving files
        grid_params (dict): Dictionary containing grid parameters
    
    Returns:
        str: Path to saved currents file
    """
    # Create filename with timestamp
    ...
    save_dir = os.path.join(base_path, year, 'currents')
    create_dir(save_dir)
    
    # Save currents
    currents_file = os.path.join(save_dir, f"secs_currents_{timestamp_str}.csv")
    
    # Create DataFrame with currents and metadata
    df_currents = pd.DataFrame({
        'current_amplitude': I_timestamp,
        'timestamp': timestamp.strftime('%Y-%m-%d %H:%M:%S'),
        'grid_center_lon': grid_params['center_lon'],
        'grid_center_lat': grid_params['center_lat'],
        'grid_orientation': grid_params['orientation'],
        'grid_shape_ns': grid_params['shape'][0],
        'grid_shape_ew': grid_params['shape'][1],
        'grid_resolution': grid_params['resolution'],
        'Rgrid': grid_params['Rgrid']
    })
    
    # Save to CSV
    df_currents.to_csv(currents_file, index=False)
    
    return currents_file